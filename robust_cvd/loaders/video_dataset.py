...
def load_image(
    ...
    if check_channels is not None:
        assert im.shape[-1] == check_channels, "receive image of shape {} whose #channels != {}".format(im.shape, check_channels)
...
class VideoDataset(data.Dataset):
    ...
    def __init__(
        ...
        if meta_file is not None:
            ...
            assert self.extrinsics.shape[0] == self.intrinsics.shape[0], "#extrinsics({}) != #intrinsics({})".format(
        ...
        if os.path.isfile(flow_list_fn):
            ...
            for [frame0, frame1, score] in self.flow_indices[1:]:
                if frame0 in frames and frame1 in frames and (min_mask_ratio is None or score > min_mask_ratio):
                    filtered.append([frame0, frame1])

            self.flow_indices = filtered
        else:
            names = os.listdir(os.path.dirname(self.flow_fmt))
            self.flow_indices = [self.parse_index_pair(name) for name in names if os.path.splitext(name)[-1] == os.path.splitext(self.flow_fmt)[-1]]
    ...
    def update_poses(self, depth_video: DepthVideo):
        ...
        for i in self.frames:
            ...
            assert descDepth.depthType == DepthXformType.Identity or descDepth.valueXform == ValueXformType.Scale
    ...
    def get_neighbor_meta(self, frame_idx, img_shape):
        ...
        if 0 < frame_idx < self.num_frames - 1:
            ...
            images_n = torch.stack(
                [load_color(self.color_fmt.format(k), channels_first=True) for k in neighbors],
                dim=0,
            )
            flows_n = [load_flow(self.flow_fmt.format(k_src, k_trg), channels_first=True) for k_src, k_trg in flow_neighbor_pair]
            masks_n = [load_mask(self.mask_fmt.format(k_src, k_trg), channels_first=True) for k_src, k_trg in flow_neighbor_pair]
        else:
    ...
    def __getitem__(self, index: int):
        ...
        if self.use_temporal_smooth_loss:
            ...
            intrinsics_n_ref, extrinsics_n_ref, images_n_ref, flows_n_ref, masks_n_ref = self.get_neighbor_meta(ref_index, (H, W))
            intrinsics_n_trg, extrinsics_n_trg, images_n_trg, flows_n_trg, masks_n_trg = self.get_neighbor_meta(trg_index, (H, W))
