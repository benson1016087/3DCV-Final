...
def visualize_depth(depth, depth_min=None, depth_max=None, cmap=colormaps.cm_magma):
    ...
    if cmap == "turbo":
        ...
        return cv2.applyColorMap(depth_scaled_uint8, cmap)
    depth_scaled = (depth - depth_min) / (depth_max - depth_min)
    depth_scaled = depth_scaled ** 0.5
    depth_scaled_uint8 = np.uint8(depth_scaled * 255)

    return ((cv2.applyColorMap(depth_scaled_uint8, cmap) / 255) ** 2.2) * 255
...
def visualize_depth_dir(
    ...
    for i in range(len(src_files)):
        ...
        if os.path.exists(f"{dst_dir}/{dst_file}") and not force:
            ...
            disparity_vis = visualize_depth(disparity, d_min, d_max, cmap=cv2.COLORMAP_TURBO)
            print(f"writing '{dst_file}'.")
            cv2.imwrite(f"{dst_dir}/{dst_file}", disparity_vis)

            # plt.figure()
            # plt.imshow(disparity, cmap="turbo", vmax=disparity.max(), vmin=disparity.min())
            # plt.axis("off")
            # plt.savefig(f"{dst_dir}/{dst_file[:-3]}_plt.png", bbox_inches="tight")
...
def create_video(pattern: str, output_file: str):
    ...
    if not os.path.exists(output_file):
        cmd = [ffmpeg, "-r", "30", "-i", pattern, "-c:v", "libx264", "-crf", "27", "-pix_fmt", "yuv420p", output_file]
        subprocess.call(cmd)
