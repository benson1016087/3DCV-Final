...
class DatasetProcessor:
    ...
    def downscale_frames(self):
        ...
        print_subbanner("Raw")
        self.video.downscale_frames("color_down", self.params.size, "raw", align=self.params.align, short_side_target=self.params.short_side_target)
    ...
    def compute_initial_depth(self):
        ...
        if not self.video.check_frames(osp.join(initial_depth_dir, "depth"), "raw"):
            ft = DepthFineTuner(self.out_dir, frames=None, base_dir=self.path, params=self.params)
    ...
    def compute_flow(self):
        ...
        full_frame_range = FrameRange(frame_range=None, num_frames=self.video.frame_count)
    ...
    def pipeline(self):
        ...
        frame_range = FrameRange(frame_range=self.params.frame_range.set, num_frames=self.video.frame_count)
        ...
        ft = DepthFineTuner(self.out_dir, frames=frames, base_dir=self.path, params=self.params)
