...
class DepthFineTuningParams:
    ...
    def add_arguments(parser):
        ...
        parser.add_argument(
            ...
            type=float,
            default=0,
            help="Learning rate for the training. If <= 0 it will be set" " automatically to the default for the specified model adapter.",
        )
...
class DepthFineTuner:
        ...
        for data in data_loader:
            ...
            for i in range(batch_size):
                ...
                if frames is None or frame_id in frames:
                    image_io.save_raw_float32_image(depth_fmt.format(frame_id) + ".raw", inv_depth)

        if self.params.save_depth_visualization:
            print(f"Visualizing depth dir: {depth_dir}...")
            with SuppressedStdout():
                visualization.visualize_depth_dir(src_dir=depth_dir, dst_dir=depth_dir, force=True)

        save_depth_end_time = time.perf_counter()
        save_depth_duration = save_depth_end_time - save_depth_start_time
        print(f"Complete saving depth for pose optimization in {save_depth_duration:.2f}s")
    ...
    def load_reference_disparity(self, frame):
        if frame not in self.reference_disparity:
            ...
            disparity = cv2.resize(disparity, (224, 384), interpolation=cv2.INTER_NEAREST)
    ...
    def fine_tune(self, writer=None):
        ...
        use_temporal_smooth_loss = (
            self.params.lambda_smooth_disparity > 0 or self.params.lambda_smooth_reprojection > 0 or self.params.lambda_smooth_depth_ratio > 0
        )
        ...
        pose_optimizer = PoseOptimizer(self.base_dir, self.params.model_type, self.frames, self.params.opt)
        ...
        def validate(epoch, niters):
            ...
            print(f"Complete Validation for epoch {epoch} ({niters} iterations) in {val_duration:.2f}s.")
        ...
        depth_names = [n for n in os.listdir(initial_depth_dir) if os.path.splitext(n)[-1] == ".raw"]
        ...
        for epoch in range(self.params.num_epochs):
            ...
            for data in train_data_loader:
                ...
                if writer is not None and total_iters % self.params.display_freq == 0:
                    write_summary(writer, "Train", stacked_img, depth, metadata, total_iters)
            ...
            if self.params.val_epoch_freq >= 0 and (epoch + 1) % self.params.val_epoch_freq == 0:
                validate(epoch + 1, total_iters)

            if self.params.save_checkpoints and (epoch + 1) % self.params.save_epoch_freq == 0:
                file_name = pjoin(self.checkpoints_dir, f"{epoch + 1:04d}.pth")
                self.model.save(file_name)

            if self.params.save_intermediate_depth_streams_freq > 0 and (epoch + 1) % self.params.save_intermediate_depth_streams_freq == 0:
                self.save_depth(frames=self.frames)

            if self.params.recon == "i3d" and (epoch + 1) % self.params.pose_opt_freq == 0:
                if self.params.save_intermediate_depth_streams_freq > 0:
        ...
        if self.params.val_epoch_freq >= 0 and self.params.num_epochs % self.params.val_epoch_freq != 0:
        ...
        print(dataset.intrinsics)
        print(dataset.extrinsics)
        torch.save(dataset.intrinsics, pjoin(self.range_dir, "intrinsic.pt"))
        torch.save(dataset.extrinsics, pjoin(self.range_dir, "extrinsic.pt"))
        print("Finished Filtering.")

    def eval_and_save(self, criterion, data_loader, epoch, niters) -> Dict[str, torch.Tensor]:
        """
        ...
        def save_eval_depth(
            ...
            for disparities, indices in zip(disparity_batch, batch_indices):
                for disparity, index in zip(disparities, indices):
                    ...
                    disparity_vis = visualization.visualize_depth(disparity, depth_min=0, depth_max=dispartiy_max)
                    cv2.imwrite(fn_pre + ".png", disparity_vis)

            return saved_frames_depth

        def save_eval_depth_xform(depth_xform_maps_batch, batch_indices, out_dir, suf, saved_frames_xform):
            ...
            for depth_xform_maps, indices in zip(depth_xform_maps_batch, batch_indices):
                for depth_xform_map, index in zip(depth_xform_maps, indices):
                    ...
                    fn_pre = pjoin(self.out_dir, "eval", "scale_{:06d}{}".format(index, suf))
        ...
        def save_scene_flow(scene_flow, batch_indices):
            ...
            for index_scene_flow in range(len(scene_flow_vis)):
                ...
                for scene_flow_vis_cur, indices in zip(scene_flow_vis_batch, batch_indices):
                    index_frame = get_scene_flow_idx(index_scene_flow, indices)
                    fn_pre = pjoin(
                        self.out_dir,
                        "eval",
                        "scene_flow_{:06d}_{:06d}_{}".format(index_frame[0], index_frame[1], suf),
                    )
        ...
        for _, data in zip(range(N), data_loader):
            ...
            batch_indices = metadata["geometry_consistency"]["indices"].cpu().numpy().tolist()
        ...
        loss_meta = {loss_name: torch.tensor(tuple(loss.values())) for loss_name, loss in loss_dict.items()}
        ...
        for pair in sorted(all_pairs):
            ...
            line += ", ".join([f"{name}: {loss_dict[name][str(pair)]:{loss_format[name]}}" for name in loss_names])
            print(line)

        print("Mean: " + " " * (2 * index_width) + ", ".join([f"{name}: {loss_dict[name][str(pair)]:{loss_format[name]}}" for name in loss_names]))
