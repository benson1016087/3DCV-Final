...
class PoseOptimizer:
    def __init__(
        ...
        if os.path.isdir(os.path.join(base_dir, "dynamic_mask")):
            self.depth_video.createColorStream("dynamic_mask", "dynamic_mask", ".png", CV_8UC1)
        ...
        if os.path.exists(f"{base_dir}/depth_gt"):
            ...
            if os.path.exists(poses_file):
                ...
                DepthVideoImporter.importPoses(self.depth_video, poses_file, gt_depth_stream)

        # Import COLMAP poses and depth stream if exists.
        if os.path.exists(os.path.join(base_dir, "colmap_dense")) and os.path.exists(os.path.join(base_dir, "depth_colmap_dense")):
            DepthVideoImporter.importColmapDepth(self.depth_video)
            self.depth_video.createDepthStream("colmap_dense", "depth_colmap_dense_imported", [-1, -1])
            colmap_file = os.path.join(base_dir, "colmap_dense/metadata.npz")
            stream = self.depth_video.depthStreamIndex("colmap_dense")
            DepthVideoImporter.importColmapRecon(self.depth_video, colmap_file, stream, False)
        ...
        self.flow_constraints = FlowConstraintsCollection(self.depth_video, flow_constraints_params)
    ...
    def duplicate_last_depth_stream(self, name, dir):
        ...
        for i in self.frames:
            shutil.copyfile(f"{src_depth_dir}/frame_{i:06d}.raw", f"{dst_depth_dir}/frame_{i:06d}.raw")
